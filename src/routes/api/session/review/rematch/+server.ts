/**
 * POST /api/session/review/rematch — replace bound SoundCloud Track + playlist.
 */
...
export const POST: RequestHandler = async ({ request }) => {
	...
	try {
		const result = await getRuntimeImportSessionApi().rematch({
		...
		console.error('[api/session/review/rematch]', error);
